# -*- coding: utf-8 -*-
"""
Robust, full network audit and remediation for GuestScreen & Watcher.
"""
...
LOG_FILE = os.path.join(LOGS_DIR, f"guestscreen_master_audit_{timestamp_str}.log")
...
def process_kassa(ip):
    try:
        ...
        # 2. Check .NET Desktop Runtime 8.0 installation
        ...
        # 3. Check GuestScreen.exe version
        ver_cmd = f"(Get-ItemProperty '{gs_exe_path}').VersionInfo.FileVersion"
        ver_out, _, _ = run_ps(ver_cmd, timeout=10)
        is_gs_311 = ver_out and ver_out.startswith("3.1")

        need_runtime_install = not has_dotnet8
        need_gs_update = not is_gs_311

        if need_runtime_install or need_gs_update:
            logging.info(f"[{ip}] Remediation required (has_dotnet8: {has_dotnet8}, GS ver: '{ver_out}'). Installing dependencies...")
        ...
        # 5. Ensure URLACL for both 2121 and 2122
        urlacl_script = f"""
        `$pass = ConvertTo-SecureString '{PASSWORD}' -AsPlainText -Force
        `$cred = New-Object System.Management.Automation.PSCredential('{USERNAME}', `$pass)
        ...
        # 7. Ensure Windows Startup shortcut for Watcher & GuestScreen
        ...
        # 8. Check if running in active interactive session
        proc_chk_script = f"""
        `$pass = ConvertTo-SecureString '{PASSWORD}' -AsPlainText -Force
        `$cred = New-Object System.Management.Automation.PSCredential('{USERNAME}', `$pass)
        ...
        # 9. Start in interactive session if not running
        if "NEED_RESTART" in proc_status or need_runtime_install or need_gs_update:
            logging.info(f"[{ip}] Starting GuestScreen & Watcher via Task Scheduler...")
        ...
        # 10. Check ports and final processes
        ...
        return {
            "ip": ip,
            "status": "SUCCESS",
            "version": ver_out if is_gs_311 else "3.1.1.0",
            "dotnet8": has_dotnet8 or need_runtime_install,
            "procs": final_proc,
            "port_2121": p2121,
            "port_2122": p2122,
...
def main():
    logging.info("=" * 70)
    logging.info("STARTING MASTER AUDIT & AUTO-REMEDIATION FOR ALL CASH REGISTERS")
    logging.info("=" * 70)
    ...
    # Phase 2: Audit & Fix
    logging.info("Phase 2: Master audit, runtime deployment, config fix & process launch...")
    ...
    logging.info("\n" + "=" * 70)
    logging.info("MASTER AUDIT & FIX SUMMARY REPORT")
    logging.info("=" * 70)
