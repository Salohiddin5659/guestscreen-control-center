# -*- coding: utf-8 -*-
"""
Full Accurate Auditor for GuestScreen across all 103 restaurant branches using Base64 Encoded PowerShell WMI.
"""
...
def check_tcp_port(ip, port=445, timeout=0.4):
    try:
...
def check_kassa_wmi(ip, filial_info):
    result = {
        "ip": ip,
    ...
    if not check_tcp_port(ip, 445, timeout=0.4) and not check_tcp_port(ip, 135, timeout=0.4):
...
def main():
    ...
    print("Опрос касс через WMI в 60 параллельных потоков...")

    start_time = time.time()
    results = []

    with ThreadPoolExecutor(max_workers=60) as executor:
        futures = {executor.submit(check_kassa_wmi, ip, f_info): ip for ip, f_info in targets}
        done_count = 0
        total = len(futures)
    ...
    for out_path in [REPORT_FILE, LATEST_REPORT]:
        with open(out_path, "w", encoding="utf-8") as f:
            ...
            if gs_running:
                f.write("| № | IP-адрес | Филиал | Префикс | Watcher | Статус |\n")
                f.write("| :- | :--- | :--- | :--- | :--- | :--- |\n")
                for idx, k in enumerate(gs_running, 1):
                    w_str = "🟢 Да" if k["watcher_running"] else "⚪ Нет"
                    f.write(f"| {idx} | `{k['ip']}` | **{k['filial_name']}** | {k['filial_prefix']} | {w_str} | 🟢 Запущен (Штатный режим) |\n")
            else:
            ...
            f.write("\n---\n\n")
            f.write("## 🏢 3. Сводная таблица по филиалам (Где есть запущенный GuestScreen)\n\n")
